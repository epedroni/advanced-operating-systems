...
errval_t send_udp_datagram(struct urpc_buffer* urpc, struct urpc_message* msg, void* context){
    ...
    debug_printf("Sending UDP datagram\n");
...
void udp_handle_connection_to_server(struct udp_parser_state* udp_state, struct udp_local_connection* local_open_connection,
    ...
    if(remote_connection==NULL){
        ...
        debug_printf("Created new remote connection, port: %lu address: 0x%04x\n", udp_packet->source_port, from);
    }
    uint8_t temp_buffer[500];
    debug_printf("Sending udp packet, data: %s\n", udp_packet->data);
    ...
    urpc_client_send_receive_fixed_size(&local_open_connection->udp_state.urpc_chan.buffer_send,
        UDP_DATAGRAM_RECEIVED, udp_commmand, len+sizeof(struct udp_command_payload_header), &udp_cmd_response, sizeof(struct udp_command_payload),&answer_size);
}
